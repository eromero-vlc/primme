...
.. f:function:: c_ptr primme_params_create()
   ...
   Eventually call :f:func:`primme_params_destroy()` to release allocated resources by PRIMME.
...
.. f:function:: c_int xprimme(evals, evecs, resNorms, primme)
   ...
   All arrays are stored on CPU, and also the computations are done on CPU (see :f:func:`magma_xprimme` for using GPUs).
