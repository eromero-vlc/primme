...
int init_basis_@(pre)primme(@(type) *V, @(type) *W, @(type) *evecs, 
   ...
   if (!primme->locking) {
      ...
      /* the user                                                     */
         ...
         update_W_@(pre)primme(V, W, 0, primme->initSize, primme);
   ...
   /*-----------------------------------------------------------------------*/
      ...
      *nextGuess = primme->numOrthoConst;
      ...
      if (primme->initSize > 0) {
         ...
         update_W_@(pre)primme(V, W, 0, currentSize, primme);
...
static int init_block_krylov(@(type) *V, @(type) *W, int dv1, int dv2, 
   ...
   if (numNewVectors <= primme->maxBlockSize) {
      ...
      update_W_@(pre)primme(V, W, dv2, 1, primme);
   ...
   /*----------------------------------------------------------------------*/
      ...
      update_W_@(pre)primme(V, W, dv2-primme->maxBlockSize+1, primme->maxBlockSize,
         primme);
