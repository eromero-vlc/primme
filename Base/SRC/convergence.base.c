...
int check_convergence_@(pre)primme(@(type) *V, @(type) *W, @(type) *hVecs, 
   ...
   while (numVacancies > 0 && 
      ...
      for (i=left; i <= right; i++) {
         Num_@(pre)copy_@(pre)primme(basisSize, &hVecs[basisSize*iev[i]], 1, 
...
static void compute_resnorms(@(type) *V, @(type) *W, @(type) *hVecs, 
   ...
   Num_gemm_@(pre)primme("N", "N", primme->nLocal, numResiduals, basisSize, 
   ...
   for (i=left; i <= right; i++) {
      ...
      ztmp = -hVals[iev[i]];
#endifarithm
      Num_axpy_@(pre)primme(primme->nLocal, ztmp, &V[primme->nLocal*(basisSize+i)],
...
#endifarithm
#ifdefarithm L_DEFREAL
      dwork[i] = Num_dot_@(pre)primme(primme->nLocal, 
...
static void swap_UnconvVecs(@(type) *V, @(type) *W, int nLocal, 
 ...
 * ievMax          Index of the next Ritz value to be targeted by the block
 ******************************************************************************/
static void check_practical_convergence(@(type) *V, @(type) *W, 
   @(type) *evecs, int numLocked, int basisSize, int blockSize, int start, 
   int numToProject, int *iev, int *flags, double *blockNorms, double tol, 
   int *recentlyConverged, int *numVacancies, @(type) *rwork, 
   primme_params *primme) {
   ...
   double normPr; 
   double normDiff;
   @(type) *overlaps;  /* Pointer in rwork to keep Q'*r */
   @(type) tpone = @(tpone), tzero = @(tzero), tmone = @(tmone); /* constants */
