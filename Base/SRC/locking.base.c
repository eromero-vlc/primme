...
int lock_vectors_@(pre)primme(double tol, double *aNormEstimate, double *maxConvTol, 
   ...
   /* -------------------------------------------------------------------- */
      if (workinW && flag[*basisSize-1] != LOCKED) {
         update_W_@(pre)primme(V, W, *basisSize-1, 1, primme);
      }
   ...
   *basisSize = *basisSize - (numDeflated + numReplaced);
   ...
   if (numReplaced > 0) {
      ...
      update_W_@(pre)primme(V, W, *basisSize, numReplaced, primme);
   }
...
static int swap_flagVecs_toEnd(int basisSize, int flagValue, @(type) *V, 
  @(type) *W, @(type) *H, double *hVals, int *flag, primme_params *primme) {
   ...
   @(type) ztmp;    /* Temporary value used for swapping                */
