...
#include "correction_private_@(pre).h"
 ...
 *
 * rwork      @(type) work array of size (primme.nLocal + 4*blockSize)
 *
 * primme       Structure containing various solver parameters
 *
 * Output parameters
...
static void Olsen_preconditioner_block(@(type) *r, @(type) *x,
   ...
   for (blockIndex = 0; blockIndex < blockSize; blockIndex++) {
      xKinvx_local[blockIndex] =
        Num_dot_@(pre)primme(primme->nLocal, &x[primme->nLocal*blockIndex],1, 
                           &Kinvx[primme->nLocal*blockIndex],1);
      xKinvr_local[blockIndex] =
        Num_dot_@(pre)primme(primme->nLocal, &Kinvx[primme->nLocal*blockIndex],1,
...
static void setup_JD_projectors(@(type) *x, @(type) *r, @(type) *evecs, 
   @(type) *evecsHat, @(type) *Kinvx, @(type) *xKinvx, 
   @(type) **Lprojector, @(type) **RprojectorQ, @(type) **RprojectorX, 
   ...
   if (primme->correctionParams.projectors.LeftQ) {
         ...
         *Lprojector = evecs;
         if (primme->correctionParams.projectors.LeftX) {
            Num_@(pre)copy_@(pre)primme(n, x, 1, &evecs[sizeEvecs*n], 1);
