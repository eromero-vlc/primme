...
int restart_dprimme(double *V, double *W, int nLocal, int basisSize, int ldV,
   ...
   double *H, int ldH, double *Q, int ldQ, double *R, int ldR,
   ...
   if (V == NULL) {
      if (primme->locking) {
         rworkSize = restart_locking_dprimme(&primme->maxBasisSize, NULL,
         ...
         rworkSize = restart_soft_locking_dprimme(&primme->maxBasisSize, NULL, NULL,
   ...
   if (!primme->locking) {
      double *X, *R;
      ret = restart_soft_locking_dprimme(&restartSize, V, W, nLocal, basisSize,
      ...
      double *X, *R;
      ret = restart_locking_dprimme(&restartSize, V, W, nLocal, basisSize,
         ldV, &X, &R, hVecs, ldhVecs, restartPerm, hVals, flags, iev, ievSize,
   ...
   restart_projection_dprimme(V, ldV, W, ldV, H, ldH, Q, ldQ, nLocal, R, ldR,
...
static int restart_soft_locking_dprimme(int *restartSize, double *V, double *W, int nLocal,
   ...
   *indexOfPreviousVecs = *restartSize;
   ...
   ret = ortho_dprimme(hVecs, ldhVecs, NULL, 0, *indexOfPreviousVecs,
   ...
   if (H == NULL) {
      ...
      return max(max(max(max(max(max(
         ...
         Num_update_VWXR_dprimme(NULL, NULL, nLocal, basisSize, 0, NULL, basisSize, 0, NULL,
            ...
            NULL, 0, 0,
            NULL, 0, primme)),
         solve_H_Ref_dprimme(NULL, 0, NULL, 0, NULL, 0, NULL, NULL, 0, NULL,
